...
<div class="container mx-auto px-6 py-10">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div onclick="openStockModal('part')" role="button" tabindex="0" class="group flex items-center justify-between w-full bg-white p-4 rounded-2xl border-2 border-purple-300 shadow-sm hover:shadow-md hover:border-purple-400 hover:-translate-y-0.5 transition-all duration-200 cursor-pointer">
            <div class="flex items-center gap-4">
                ...
                <div class="text-left">
                    <h4 class="text-base font-bold text-gray-800 group-hover:text-purple-700 leading-tight">Find Seq No</h4>
    ...
    <div id="stockModal" class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 hidden opacity-0 transition-opacity duration-300">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-5xl h-[80vh] flex flex-col transform scale-95 transition-transform duration-300 mx-4">
            ...
            <div class="p-4 border-b border-gray-200 bg-gray-50 flex gap-2 flex-shrink-0">
                <input type="text" id="stockSearchInput" class="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="Type to search...">
                <button id="btnStockSearch" class="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded-lg font-bold transition-colors shadow-sm">SEARCH</button>
            </div>
            ...
            <div class="p-3 border-t border-gray-200 bg-gray-50 text-right rounded-b-2xl text-xs text-gray-500">PJVK Warehouse System</div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden">
        <div class="bg-gradient-to-r from-purple-900 via-violet-800 to-indigo-800 px-6 py-5 flex flex-col md:flex-row justify-between md:items-center gap-4 shadow-inner">
            ...
            <div class="flex flex-wrap gap-2 justify-end w-full md:w-auto items-center">
                <div class="flex items-center bg-white/10 rounded-lg border border-white/10 p-0.5 w-full md:w-auto">
                    <select id="searchColumn" class="bg-transparent text-white text-xs font-bold border-none focus:ring-0 cursor-pointer py-2 pl-2 pr-7 outline-none [&>option]:text-gray-900">
                        <option value="all">ALL</option>
                        <option value="id_code">ID</option>
                        <option value="part_no">PART NO</option>
                        <option value="erp_code">ERP CODE</option>
                        <option value="seq_no">SEQ NO</option>
                        <option value="racking_location">LOCATION</option>
                    </select>
                    <div class="h-4 w-px bg-white/20 mx-1"></div>
                    <div class="relative w-full md:w-48">
                        <input type="text" id="historySearchInput" placeholder="Type to search..." class="w-full bg-transparent text-white text-sm border-none focus:ring-0 placeholder-purple-200 outline-none pl-2 py-1" />
                    </div>
                    <button id="triggerSearchBtn" class="pr-3 pl-1 text-purple-300 hover:text-white transition-colors">
                        <i data-lucide="search" class="w-4 h-4"></i>
                    </button>
                </div>
                <div class="flex items-center gap-1 bg-white/10 p-1 rounded-lg border border-white/10">
                    <select id="dateColumn" class="bg-transparent text-white text-xs font-bold border-none focus:ring-0 cursor-pointer py-1 pl-1 pr-6 outline-none [&>option]:text-gray-900" title="Filter by...">
                        <option value="DATE_IN">Date In</option>
                        <option value="RECEIVING_DATE">Rec. Date</option>
                    </select>
                    <div class="h-4 w-px bg-white/20 mx-1"></div>
                    <input type="date" id="dateFrom" class="bg-transparent text-white text-xs border-none focus:ring-0 px-1 py-1 w-24" title="From">
                    <span class="text-purple-200 text-xs">to</span>
                    <input type="date" id="dateTo" class="bg-transparent text-white text-xs border-none focus:ring-0 px-1 py-1 w-24" title="To">
                </div>
                <button id="refreshBtn" class="p-2 rounded-lg bg-white/20 hover:bg-white/30 text-white transition-all shadow-md" title="Reset Filters"><i data-lucide="refresh-cw" class="w-5 h-5"></i></button>
            ...
            <table id="scanHistoryTable" class="w-full text-sm">
                <thead class="sticky top-0 z-10 bg-gray-100 border-b border-gray-200"> 
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left font-bold text-gray-600 uppercase">Status</th>
                ...
                <tbody id="scanHistoryTableBody" class="bg-white divide-y divide-gray-200">
                    <tr><td colspan="11" class="text-center py-10 text-gray-500">Loading...</td></tr>
                </tbody>
            </table>
        </div> 
...
<div id="manualEntryModal" class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 opacity-0 invisible transition-opacity duration-300">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 transform scale-95 transition-transform duration-300 hover:scale-100 flex flex-col overflow-hidden">
        <div class="bg-gradient-to-r from-purple-800 to-indigo-700 px-6 py-4 flex items-center justify-between flex-shrink-0">
            ...
            <button type="button" id="closeManualModal" class="text-white/80 hover:text-white transition-colors"><i data-lucide="x" class="w-6 h-6"></i></button>
        </div>
        <form id="manualEntryForm" class="p-5">
            ...
            <div id="manualDetailsContainer" class="hidden bg-slate-50 p-3 rounded-lg border border-slate-200 mb-4 shadow-inner">
                <div class="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                    ...
                    <div><span class="text-xs text-gray-400 block uppercase">Part No</span><span id="manual_part_no_fg" class="font-bold text-gray-800"></span></div>
                    <div><span class="text-xs text-gray-400 block uppercase">ERP Code</span><span id="manual_erp_code_b" class="font-bold text-gray-800"></span></div>
                    <div><span class="text-xs text-gray-400 block uppercase">Seq No</span><span id="manual_seq_no" class="font-bold text-gray-800"></span></div>
                    <div><span class="text-xs text-gray-400 block uppercase">Qty</span><span id="manual_rack_in" class="font-bold text-emerald-600 text-base"></span></div>
                    <div class="col-span-2 pt-1 border-t border-gray-200 mt-1"><div class="flex justify-between items-center"><span class="text-xs text-gray-400 uppercase">Rec Date</span><span id="manual_receiving_date" class="font-bold text-gray-800"></span></div></div>
                </div>
            </div>
            ...
            <div class="flex items-center justify-end space-x-3 pt-2 border-t border-gray-100">
                ...
                <button type="button" id="submitManualEntry" class="px-4 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 text-white text-sm font-bold rounded-lg hover:from-purple-700 hover:to-indigo-700 shadow-md disabled:opacity-50 transition-all" disabled><i data-lucide="save" class="w-4 h-4 inline-block mr-1"></i> Confirm</button>
            </div>
        </form>
    </div>
</div>
...
<div id="masterListModal" class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 hidden opacity-0 transition-opacity duration-300">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-6xl h-[85vh] flex flex-col transform scale-95 transition-transform duration-300 mx-4">
        <div class="bg-gradient-to-r from-amber-600 to-orange-600 px-6 py-4 rounded-t-2xl flex justify-between items-center flex-shrink-0">
            <h3 class="text-xl font-bold text-white flex items-center gap-2"><i data-lucide="list" class="w-5 h-5"></i> <span>Master Incoming List</span></h3>
            <div class="flex items-center gap-2">
                <button onclick="openMasterInput('add')" class="bg-white/20 hover:bg-white/30 text-white p-2 rounded-lg transition-colors flex items-center gap-2 font-bold text-xs border border-white/20"><i data-lucide="plus" class="w-4 h-4"></i> ADD NEW</button>
                <button onclick="closeMasterModal()" class="text-white/70 hover:text-white transition-colors p-1"><i data-lucide="x" class="w-6 h-6"></i></button>
            </div>
        </div>
        <div class="p-4 border-b border-gray-200 bg-gray-50 flex gap-2 flex-shrink-0">
            ...
            <button id="btnMasterSearch" class="bg-gray-800 hover:bg-gray-900 text-white px-6 py-2 rounded-lg font-bold transition-colors shadow-sm">SEARCH</button>
        </div>
        <div class="flex-1 overflow-y-auto custom-scrollbar p-0">
            <table class="w-full text-sm text-left">
                <thead class="bg-gray-800 text-white uppercase font-bold sticky top-0 z-10 shadow-sm"><tr><th class="px-4 py-3">Supplier</th><th class="px-4 py-3">Seq No</th><th class="px-4 py-3">Part No</th><th class="px-4 py-3">ERP Code</th><th class="px-4 py-3">Description</th><th class="px-4 py-3 text-center">Std Pack</th><th class="px-4 py-3 text-center">Action</th></tr></thead>
                <tbody id="masterTableBody" class="divide-y divide-gray-200"><tr><td colspan="7" class="text-center py-10 text-gray-400">Loading master list...</td></tr></tbody>
            </table>
        </div>
        <div class="p-3 border-t border-gray-200 bg-gray-50 text-right rounded-b-2xl text-xs text-gray-500">PJVK Warehouse System</div>
    </div>
...
<div id="masterInputModal" class="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-[60] hidden opacity-0 transition-opacity duration-300">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg transform scale-95 transition-transform duration-300 mx-4 border-2 border-amber-500">
        <div class="bg-amber-50 px-6 py-4 rounded-t-xl border-b border-amber-100 flex justify-between items-center"><h3 class="text-lg font-bold text-amber-800" id="masterInputTitle">Edit Master Data</h3><button onclick="closeMasterInput()" class="text-gray-400 hover:text-gray-600"><i data-lucide="x" class="w-5 h-5"></i></button></div>
        <div class="p-6 space-y-4">
            <input type="hidden" id="mi_original_part_no"> 
            <div class="grid grid-cols-2 gap-4"><div><label class="block text-xs font-bold text-gray-500 uppercase mb-1">Supplier</label><select id="mi_supplier" class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-amber-500 font-bold text-gray-700"><option value="YTEC">YTEC</option><option value="MC">MC</option></select></div><div><label class="block text-xs font-bold text-gray-500 uppercase mb-1">Seq No</label><input type="number" id="mi_seq" class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-amber-500"></div></div>
            <div class="grid grid-cols-2 gap-4"><div><label class="block text-xs font-bold text-gray-500 uppercase mb-1">Part No</label><input type="text" id="mi_part_no" class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-amber-500 uppercase"></div><div><label class="block text-xs font-bold text-gray-500 uppercase mb-1">ERP Code</label><input type="text" id="mi_erp" class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-amber-500 uppercase"></div></div>
            <div><label class="block text-xs font-bold text-gray-500 uppercase mb-1">Part Description</label><input type="text" id="mi_desc" class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-amber-500 uppercase"></div>
            <div><label class="block text-xs font-bold text-gray-500 uppercase mb-1">Std Packing</label><input type="number" id="mi_pack" class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-amber-500"></div>
        </div>
        <div class="p-4 border-t bg-gray-50 rounded-b-xl flex justify-end gap-2"><button onclick="closeMasterInput()" class="px-4 py-2 text-gray-600 hover:bg-gray-200 rounded-lg font-bold text-sm">Cancel</button><button onclick="saveMasterData()" id="btnSaveMaster" class="px-6 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-lg font-bold text-sm shadow-md transition-all">SAVE DATA</button></div>
    </div>
</div>
