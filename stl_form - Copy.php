...
function sendTelegramMessage($chat_id, $message) {
    ...
    curl_setopt($ch, CURLOPT_TIMEOUT, 5); // 5-second timeout so it doesn't freeze the page
    $result = curl_exec($ch);
    curl_close($ch);
...
if (isset($_GET['export_csv'])) {
    ...
    if ($order) {
        ...
        header('Content-Disposition: attachment; filename=STL_Record_Trip_' . $order['trip_number'] . '_' . date('Ymd') . '.csv');
        $output = fopen('php://output', 'w');
        
        fputcsv($output, ['STL DIGITAL FORM - OFFICIAL RECORD']);
        fputcsv($output, ['Model:', $order['model'], 'Line:', $order['line']]);
        fputcsv($output, ['FG/Sub-Assy:', $order['fg_code'], 'Variance:', $order['variance']]);
        fputcsv($output, ['Trip:', $order['trip_number'], 'Shift:', $order['shift']]);
        fputcsv($output, ['Status:', $order['status'], 'Date:', $order['requested_at']]);
        fputcsv($output, []); // blank row
        
        fputcsv($output, ['Bil', 'Part Name', 'Part No', 'ERP Code', 'Area', 'Usage', 'Req Qty', 'Sup Qty', 'Rec Qty']);
        
        // Dynamic Area Override for Line 5 CSV
        ...
        $stmt_sup = $pdo->prepare("SELECT sup_qty FROM stl_supply WHERE order_id = ? ORDER BY id DESC LIMIT 1");
        ...
        foreach ($parts as $p) {
            ...
            fputcsv($output, [$bil++, $p['stock_desc'], $p['part_no'], $p['erp_code'], $p['area'], $p['usage'], $r, $s, $rc]);
        }
        fclose($output);
        exit;
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action == 'upload_part_image') {
        header('Content-Type: application/json');
        $response = ['success' => false, 'message' => 'Unknown error.'];

        if (isset($_FILES['part_image']) && isset($_POST['part_id'])) {
            $p_id = $_POST['part_id'];
            $file = $_FILES['part_image'];
            if ($file['error'] === UPLOAD_ERR_OK) {
                ...
                if (in_array($ext, $allowed_exts)) {
                    $newFileName = 'part_' . $p_id . '_' . time() . '.' . $ext;
                    $destPath = $uploadDir . $newFileName;
                    if (move_uploaded_file($file['tmp_name'], $destPath)) {
                        $stmt_img = $pdo->prepare("UPDATE master_stl SET img = ? WHERE id = ?");
                        if ($stmt_img->execute([$destPath, $p_id])) {
                            $response['success'] = true; $response['filepath'] = $destPath;
                        } else { $response['message'] = 'Database update failed.'; }
                    } else { $response['message'] = 'Failed to move uploaded file.'; }
                } else { $response['message'] = 'Invalid file type.'; }
            } else { $response['message'] = 'Upload error code: ' . $file['error']; }
        } else { $response['message'] = 'No file provided.'; }
    ...
    elseif ($action == 'create_request') {
        ...
        $stmt->execute([$model, $line, $_POST['fg_code'], $_POST['variance'] ?? '', $_POST['trip_number'], $_POST['shift'], $current_day_name, $_POST['staff_name'], $_POST['verify_name'], $_POST['signature_data'], $req_json]);
        
        // --- TELEGRAM NOTIFICATION (PROD TO MPL) ---
        // ⚠️ REPLACE "-123456789" with your MPL Group Chat ID or PIC Chat ID
        $mpl_chat_id = "1591433244"; 
        ...
        echo "<script>window.location.href='stl_form.php?success=1&fg=".urlencode($_POST['fg_code'] ?? '')."&sub=".urlencode($_POST['sub_assy'] ?? 'ALL')."&variance=".urlencode($_POST['variance'] ?? '')."';</script>"; exit;
    } 
    elseif ($action == 'advance_to_receive') {
        ...
        $stmt->execute([$_POST['order_id'], $current_day_name, $_POST['staff_name'], $_POST['verify_name'], $_POST['shift'], $_POST['signature_data'], $sup_json]);

        // --- TELEGRAM NOTIFICATION (MPL TO PROD) ---
        // ⚠️ REPLACE "-987654321" with your Production Group Chat ID or PIC Chat ID
        $prod_chat_id = "756553229"; 
        
        $msg = "✅ *STL SUPPLY READY*\n";
        $msg .= "Order ID: " . $_POST['order_id'] . "\n";
        $msg .= "Model: $model | Line: $line\n";
        $msg .= "MPL has prepared the parts. Please proceed to receive.\n";
        $msg .= "Supplied By: " . $_POST['staff_name'];
    ...
    elseif ($action == 'advance_to_complete') {
        ...
        $stmt = $pdo->prepare("UPDATE stl_orders SET status = 'COMPLETED', received_at = NOW(), received_by = ?, recv_signature = ?, rec_qty = ? WHERE id = ?");
        $stmt->execute([$_POST['staff_name'], $_POST['signature_data'], $rec_json, $_POST['order_id']]);

        // --- TELEGRAM NOTIFICATION (JOB COMPLETED -> NOTIFY BOTH) ---
        // ⚠️ Replace these with your actual Chat IDs
        $mpl_chat_id = "-123456789";  
        $prod_chat_id = "-987654321"; 
...
if (isset($_GET['order_id'])) {
    ...
    if($active_task) {
        ...
        $_GET['sub'] = 'ALL'; // Reset sub-assy to ALL when viewing order details
        $_GET['variance'] = $active_task['variance'];
    }
}

// --- VIRTUAL LINE OVERRIDE LOGIC ---
// If viewing Line 5, include NW & PNE. If viewing other lines, hide NW & PNE.
$line_cond = ($line == '5') 
    ? "(line1 = ? OR UPPER(area) IN ('NW', 'PNE'))" 
    : "(line1 = ? AND UPPER(area) NOT IN ('NW', 'PNE'))";
...
// 2. Fetch Sub-Assemblies based on the selected FG Code
$sub_assy_list = [];
if ($selected_fg) {
...
$selected_sub = $_GET['sub'] ?? 'ALL'; // Default to ALL to combine sub-assemblies

// 3. Fetch Variances based on FG and (optionally) Sub-Assy
$variances = [];
if ($selected_fg) {
                        ...
                        <?php else: ?>
                            ...
                            <a href="stl_form.php?order_id=<?php echo $task['id']; ?>" class="task-card min-w-[280px] shrink-0 block p-5 rounded-2xl border-2 <?php echo $bg; ?> transition-all duration-300 snap-start" data-date="<?php echo $task_date; ?>" data-trip="<?php echo $task['trip_number']; ?>">
                                <div class="flex justify-between items-start mb-2">
                                    <h4 class="font-black text-slate-800 text-lg flex items-center gap-2">TRIP <?php echo $task['trip_number']; ?> <span class="text-[10px] bg-white border border-slate-200 px-2 py-1 rounded-lg uppercase shadow-sm"><?php echo $task['shift']; ?></span></h4>
                                </div>
                                ...
                                <div class="flex flex-wrap gap-2">
                                    ...
                                    <?php if($task['variance']): ?><span class="text-[11px] font-black text-<?php echo $theme['color']; ?>-700 bg-<?php echo $theme['color']; ?>-100 px-2.5 py-1 rounded-md border border-<?php echo $theme['color']; ?>-200"><?php echo htmlspecialchars($task['variance']); ?></span><?php endif; ?>
                                </div>
                            </a>
                            <?php endforeach; ?>
                        <?php endif; ?>
